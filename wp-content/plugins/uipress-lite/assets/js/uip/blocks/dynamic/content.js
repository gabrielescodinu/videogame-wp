  ...
  return {
    ...
    template: `<!--builder mode-->
      <div ref="frameContainer" class="uip-flex uip-flex-column uip-overflow-hidden uip-content-frame uip-overflow-hidden uip-position-relative" :class="returnClasses" :id="block.uid">
        ...
        <iframe v-if="uiTemplate.display == 'prod'" ref="contentframe" class="uip-page-content-frame uip-background-default uip-scrollbar uip-w-100p uip-flex-grow"></iframe>
        <iframe v-else :src="startPage" ref="contentframe" class="uip-page-content-frame uip-background-default uip-scrollbar uip-w-100p uip-flex-grow"></iframe>
        ...
        <template v-else>
          ...
          <div @mouseover="cornertickle = true" @mouseleave="cornertickle = false" class="uip-fullscreen-toggle uip-position-absolute uip-text-muted uip-top-0 uip-right-0 uip-cursor-pointer uip-translate-all-back-50p uip-w-60 uip-ratio-1-1 uip-flex uip-flex-column uip-flex-middle uip-padding-xs ">
            
            <div @click="toggleFullScreen()" v-if="cornertickle" class="uip-border-box uip-background-muted uip-text-muted uip-padding-xxxs uip-cursor-pointer hover:uip-background-grey uip-w-60 uip-ratio-1-1 uip-flex uip-flex-column uip-flex-middle uip-fade-in">
              <span class="uip-icon uip-text-xl uip-slide-in-right">arrow_back</span>
            </div>
